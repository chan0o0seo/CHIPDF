...
The macOS build uses the upstream tesserocr 2.10.0 wheel and its bundled native libraries instead of the Windows OCR wheel. Its package license and native-library notices are collected into the app's `release-info/licenses/python-packages` directory. PySide6 and all other native packages are built or installed for the target Mac architecture; the Windows DLLs are not redistributed in the Mac app.
