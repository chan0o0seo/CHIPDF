...
The macOS build uses upstream tesserocr 2.10.0 instead of the Windows OCR wheel. Intel builds compile the binding against Homebrew Tesseract to keep its C API compatible with the installed cysignals. Python package notices are collected into `release-info/licenses/python-packages`; Homebrew native-library notices are collected separately when applicable. PySide6 and other native packages target the corresponding Mac architecture; Windows DLLs are not redistributed in the Mac app.

The optional `--with-inpaint` build adds ONNX Runtime (MIT) and the Carve conversion of Big-LaMa. Model source: https://huggingface.co/Carve/LaMa-ONNX (revision `a3ee2fca54baebec351b8fa7786154ffa7555aa6`, model card declares Apache-2.0); original research: https://github.com/advimman/lama. The fixed model SHA256 is `1faef5301d78db7dda502fe59966957ec4b79dd64e16f03ed96913c7a4eb68d6`. Runtime package notices are copied by the build script. The Apache-2.0 license with upstream Samsung Research attribution is included as `vendor/notices/LaMa-Apache-2.0.txt` (upstream: https://github.com/Carve-Photos/lama/blob/main/LICENSE) and copied into the release licenses folder. The downloaded ONNX file is bundled without modification, with its recorded origin.
